...
// Screens subscribe here to learn when a pull has applied remote changes to
// SQLite, so they can re-read and re-render. This is what makes realtime
// updates appear without a manual focus/reload.
...
function notifyUpdated() {
  for (const cb of updateListeners) {
    try {
      ...
      if (__DEV__) console.log('[sync] listener error:', e.message);
    }
  }
}
...
export async function syncNow(userId) {
  ...
  try {
    await pushChanges();
    await pullChanges();
    // Let any focused screen re-read SQLite (covers realtime-triggered syncs).
    notifyUpdated();
  } catch (e) {
